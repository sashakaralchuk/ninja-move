...
std::string ClientPublic::gen_tickers_insert_sql(std::string table_name) {
    if (template_tickers_insert_sql == "") {
        throw std::runtime_error("template_tickers_insert_sql is empty");
    }
    return fmt::format(template_tickers_insert_sql, table_name);
}
...
void ClientPrivateGateio::init_exchange_info() {
    ...
    if (kind == "fut") {
        ...
        fut_exchange_info = exec_http_get_req(url_str);
    } else if (kind == "spot") {
        std::string url_str =
...
Order ClientPrivateGateio::fetch_order(Order& order) {
    ...
    if (kind == "fut") {
        ...
        nlohmann::json res_obj = exec_http_get_req(url_path, headers);
        SPDLOG_DEBUG("{} {} res_obj={}", ex, kind, res_obj.dump());
        Order order = Order{
            .ex = ex,
            .k = kind,
...
Order ClientPrivateMexc::fetch_order(Order& order) {
    if (kind == "spot") {
        ...
        nlohmann::json res_obj = exec_http_get_req(url, headers);
        SPDLOG_DEBUG("{} {} res_obj={}", ex, kind, res_obj.dump());
        Order order = Order{
            .ex = ex,
            .k = kind,
...
ClientPublicBybit::ClientPublicBybit(std::string kind)
    ...
    if (kind == "fut") {
        template_tickers_insert_sql = R"(
            ...
            FROM (
                SELECT
                    arrayJoin(JSONExtractArrayRaw(JSONExtractRaw(json, 'result'), 'list')) obj_raw,
...
void ClientPrivateBybit::init_exchange_info() {
    ...
    if (kind == "fut") {
        fut_exchange_info = exec_http_get_req(fut_ex_info_url_str);
    } else if (kind == "spot") {
...
std::vector<FundingRes> ClientPublicHyperliquid::fetch_fundings_sync() {
    ...
    for (int i = 0; i < res_obj[1].size(); i++) {
        ...
        fundings_vec.push_back(FundingRes{
            .obj_raw = nlohmann::json{{"meta", meta_obj},
                                      {"assert_ctx", assert_ctx_obj}},
            .obj_k = "info",
            .s = fmt::format("{}_USDC", (std::string)meta_obj["name"]),
...
ClientPublicParadex::ClientPublicParadex(std::string kind)
    ...
    template_fundings_insert_sql = R"(
        ...
        SELECT
            ...
            JSONExtractString(obj_raw, 'symbol') s,
            JSONExtractFloat(obj_raw, 'funding_rate') funding_rate,
            JSONExtract(obj_raw, 'created_at', 'UInt64') ts,
            'paradex' ex,
            'fut' k,
            NOW() ts_write
        FROM (
            SELECT
                arrayJoin(JSONExtractArrayRaw(json, 'results')) obj_raw
...
ClientPublicPolynomialFi::ClientPublicPolynomialFi(std::string kind)
    ...
    template_fundings_insert_sql = R"(
        ...
        SELECT
            ...
            'market' obj_k,
            JSONExtractString(obj_raw, 'symbol') s,
            JSONExtractFloat(obj_raw, 'currentFundingRate1HInPercentage') funding_rate,
            NOW() ts,
            'polynomial-fi' ex,
            'fut' k,
            NOW() ts_write
        FROM (
            SELECT arrayJoin(JSONExtractArrayRaw(json, 'markets')) obj_raw
            FROM url('https://perps-api-mainnet.polynomial.finance/markets', 'JSONAsString')
        );
    )";
}
...
std::vector<TickerRes> ClientPublicApexPro::fetch_tickers_sync() {
...
std::vector<TickerRes> ClientPublicApexOmni::fetch_tickers_sync() {
    std::string url_assets = "https://omni.apex.exchange/api/v3/symbols";
    nlohmann::json res_symbols_obj = exec_http_get_req(url_assets);
    std::vector<TickerRes> tickers_vec;
    long ts = now_millis();
    auto perpetualContract =
        res_symbols_obj["data"]["contractConfig"]["perpetualContract"];
    SPDLOG_INFO("ex={} k={} start fundings upload size={}", ex, k,
                perpetualContract.size());
    for (auto& obj_symbol : perpetualContract) {
        std::string url =
...
std::vector<FundingRes> ClientPublicAevo::fetch_fundings_sync() {
    ...
    for (std::string asset : res_assets_obj) {
        ...
        fundings_vec.push_back(FundingRes{
            .obj_raw = nlohmann::json({}),
            .obj_k = "funding",
            .s = fmt::format("{}-USD", asset),
            .funding_rate = stod((std::string)res_obj["funding_rate"]),
            .ts = ts,
            .ex = ex,
            .k = k,
        });
...
ClientPublicBitunix::ClientPublicBitunix(std::string kind) {
    ex = "bitunix";
    k = kind;
}

std::vector<FundingRes> ClientPublicBitunix::fetch_fundings_sync() {
    std::string url_symbols =
        "https://fapi.bitunix.com/api/v1/futures/market/tickers";
    nlohmann::json res_sumbols_obj = exec_http_get_req(url_symbols);
    std::vector<FundingRes> fundings_vec;
    long ts = now_millis();
    SPDLOG_INFO("ex={} k={} start fundings upload size={}", ex, k,
                res_sumbols_obj["data"].size());
    for (auto& symbol_obj : res_sumbols_obj["data"]) {
        std::string s = symbol_obj["symbol"];
        std::string url = fmt::format(
        ...
        fundings_vec.push_back(FundingRes{
            .obj_raw = res_obj["data"],
            .obj_k = "funding-rate",
            .s = s,
            .funding_rate = stod((std::string)res_obj["data"]["fundingRate"]),
            .ts = ts,
            .ex = ex,
            .k = k,
        });
...
void ClientPublicCoinEx::subscribe_to_trades(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicCoinEx::subscribe_to_depth(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicCoinEx::unsubscribe_from_depth(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicCoinEx::ping() { throw std::runtime_error("not-implemented"); }

Ticker ClientPublicCoinEx::fetch_ticker(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

std::vector<Ticker> ClientPublicCoinEx::fetch_tickers() {
    throw std::runtime_error("not-implemented");
}

void ClientPublicCoinEx::handle_onmessage(const std::string& msg) {
    throw std::runtime_error("not-implemented");
}
...
void ClientPublicBingx::subscribe_to_trades(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBingx::subscribe_to_depth(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBingx::unsubscribe_from_depth(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBingx::ping() { throw std::runtime_error("not-implemented"); }

Ticker ClientPublicBingx::fetch_ticker(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

std::vector<Ticker> ClientPublicBingx::fetch_tickers() {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBingx::handle_onmessage(const std::string& msg) {
    throw std::runtime_error("not-implemented");
}

ClientPublicBinance::ClientPublicBinance(std::string kind)
    : ClientPublic("binance", kind) {
    if (kind == "fut") {
        template_tickers_insert_sql = R"(
            INSERT INTO {}
            SELECT
...
void ClientPublicBinance::init_idle() {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBinance::subscribe_to_trades(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBinance::subscribe_to_depth(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBinance::unsubscribe_from_depth(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBinance::ping() {
    throw std::runtime_error("not-implemented");
}

Ticker ClientPublicBinance::fetch_ticker(std::string& symbol) {
    throw std::runtime_error("not-implemented");
}

std::vector<Ticker> ClientPublicBinance::fetch_tickers() {
    throw std::runtime_error("not-implemented");
}

void ClientPublicBinance::handle_onmessage(const std::string& msg) {
    throw std::runtime_error("not-implemented");
}
